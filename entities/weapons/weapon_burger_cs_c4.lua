...
SWEP.Primary.Sound			= Sound("weapons/c4/c4_initiate.wav")
SWEP.Primary.Cone			= 0
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		if not self:AllowedToPlant(true) then
			self.Owner:ChatPrint("You are not at a bomb site.")
			return
		end
	end

	self:EmitSound("weapons/c4/c4_initiate.wav")
...
function SWEP:AllowedToPlant(reset)
	...
	-- 	return false
	-- end
	if(self:GetNWBool("canplant",false) == true) then
		return true
	else
		return false
	end
